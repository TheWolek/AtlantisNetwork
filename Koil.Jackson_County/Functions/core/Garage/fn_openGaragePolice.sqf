...
if(myJob == "Cop") then {
	...
	if(packet < 100000) then {
		...
		if(player getvariable "cop" >= 1 && typeOf _house == "Land_PoliceStation") then {
			_garage pushback format ["VVV_dodge_charger_pd_%1",_num];
			_garage pushback format ["VVV_dodge_charger_marshals_%1",_num];
			_garage pushback format ["VVV_dodge_charger_highway_%1",_num];
		};
...
if(myJob == "doc") then {
	...
	if(packet < 100000) then {
		...
		if(player getVariable "doc" >= 1 && typeOf _house == "Land_PoliceStation") then {
			_garage pushback format ["VVV_dodge_charger_doc_%1",_num];
		};
		...
		if(player getVariable "doc" >= 1 && typeOf _house == "Land_PoliceStation") then {
			_garage pushback "VVV_dodge_charger_doc";
		};
...
if(myJob == "EMS") then {
	...
	if(player getvariable "ems" >= 6 && (typeof _house IN ['Land_buildingsfiredept1','Land_buildingshospital1'])) then {
		...
		_garage pushback "nopixel_guardacostas";
	};
