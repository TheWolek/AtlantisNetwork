...
if!(_cashCheck) exitwith { [format["%1 - Nie masz dość pieniędzy, aby za to zapłacić!",(_price*(1+taxRate/100)) call client_fnc_numberText], true] spawn domsg; };

if( ((player getvariable "cashinhand") - _price*(1+taxRate/100)) < (client_level_array select 16) ) exitWith {
    [format["Sklep samochodowy nie akceptuje Twojego %1 w brudnych pieniądzach.", (client_level_array select 16) call client_fnc_numberText], true] spawn domsg;
};

[_price*(1+taxRate/100), true] call Client_fnc_removeCash;
["govtBank", _price*(taxRate/100), "Add"] remoteExec["server_fnc_setValue",2];
["govtBank", format["%1 (%2) dodano %3 do rządowego konta bankowego.", name player, getplayeruid player, (_price*(taxRate/100)) call client_fnc_numberText]] remoteExec ["server_fnc_log",2];
