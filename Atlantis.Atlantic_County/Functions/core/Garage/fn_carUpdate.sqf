...
if (vehicle player == player) exitwith { ["Musisz być w pojeździe, aby go ulepszyć!", true] spawn domsg; };
...
if(_attempt && _oldveh in current_cars) then {
	...
	["Ten pojazd jest już ulepszony, lub nie można go ulepszyć!", true] spawn domsg;
};