...
*/
	...
	*/
	...
	[
		[" (CursorTarget isKindOf 'Air' || CursorTarget isKindOf 'Car')  && CursorTarget in Current_Cars && CursorTarget in currentcopsquadcars && (myJob IN ['Cop','doc'] && count (nearestObjects[player,['land_jf_sheriff_station_01'],50]) > 0 ) && !imrestrained && !attachedcar"],
		["Schowaj radiowóz", "[Chowam pojazd',10, 'Client_fnc_storeCar',player,'AinvPknlMstpSnonWnonDnon_medic_1',CurrentCursorTarget,'cg_sndimg\sounds\repair.ogg'] spawn client_fnc_dotask",2]
	],
	
	[
		[" (CursorTarget isKindOf 'Air' || CursorTarget isKindOf 'Car') && CursorTarget in Current_Cars && ( count (cursorTarget getvariable ['information',[]]) == 0 && myJob != 'none' ) && !imrestrained && !(CursorTarget in currentcopsquadcars)"],
	...
	[
		[" (CursorTarget isKindOf 'Boat' || CursorTarget isKindOf 'Ship') && CursorTarget in Current_Cars && !imrestrained && count (nearestterrainobjects[player,['BUILDING'],50] select {str _x find 'embarcadero' > -1}) > 0 && !(CursorTarget in currentcopsquadcars) "],
