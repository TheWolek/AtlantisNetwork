...
if((_height select 2) > 15) exitwith { ["Musisz być na ziemi, aby tego użyć!", true] spawn domsg; };
...
if(count tower > 0) then {
	if(player distance (tower select 0) < 16) then { ["Jesteś za blisko", true] spawn domsg; };
	godMode = true;
	_speed = 115;
	...
	["Jesteś za daleko, żeby zaczepić się o wieżę!", true] spawn domsg;
};