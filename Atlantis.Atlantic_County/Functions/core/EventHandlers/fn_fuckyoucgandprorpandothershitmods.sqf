...
//junky drink
if (_item IN ["np_coke","np_drpepper"]) then { ["add","drink",100] call client_fnc_sustain; [_item,0] spawn client_fnc_removeitem; closeDialog 0; };
...
if (_item IN 
	...
	) then { ["add","drink",60] call client_fnc_sustain; ["add","food",60] call client_fnc_sustain; [_item,20] spawn client_fnc_removeitem; closeDialog 0; };

if (_item IN 
	["np_chdonuts","np_jdonuts","np_cdonuts","np_donuts"]
	) then { ["add","drink",60] call client_fnc_sustain; ["add","food",60] call client_fnc_sustain; if (myjob != "cop" && myjob != "doc") then { } else { ["donuts",120] spawn client_fnc_useDrug; };  [_item,20] spawn client_fnc_removeitem; closeDialog 0; };

if (_item IN ["np_ugroceries"] ) then { ["add","drink",100] call client_fnc_sustain; ["add","food",100] call client_fnc_sustain;  [_item,80] spawn client_fnc_removeitem; closeDialog 0; };
