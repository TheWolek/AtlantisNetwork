...
if(isEngineOn _vehicle) exitWith { ["Samochód musi być wyłączony by go zatankować.", true] spawn domsg; };
if(player IN _vehicle) exitWith { ["Nie możesz być w pojeździe.", true] spawn domsg; };
...
while {true} do {
	...
	if!(_cashCheck) exitwith { [format["%1 - Nie masz wystarczająco pieniędzy by zapłacić!", _price call client_fnc_numberText], true] spawn domsg; };
	...
	if(_curFuel > 0.99 || isEngineOn _vehicle || _vehicle distance _startpos >= 5) exitwith { 
		if(isEngineOn _vehicle) then { ["Tankowanie anulowane, ponieważ został włączony silnik.", true] spawn domsg; };
		
		[format["Pojazd został zatankowany za %1.",_total call client_fnc_numberText], true] spawn domsg; 
