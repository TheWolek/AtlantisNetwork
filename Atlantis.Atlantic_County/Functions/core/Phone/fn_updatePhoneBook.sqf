disableSerialization;
	...
	for "_i" from 0 to 20 step 1 do {
		_pia =  currentDoc FIND objNull;  
		 currentDoc deleteAT _pia;  
	};

	if!(_old isEqualTo  currentDoc) then { publicvariable "currentDoc"; };
	...
	for "_i" from 0 to 20 step 1 do {
		_pia =  currentBiker FIND objNull;  
		 currentBiker deleteAT _pia;  
	};

	if!(_old isEqualTo  currentBiker) then { publicvariable "currentBiker"; };


	_old =  currentMobster;

	for "_i" from 0 to 20 step 1 do {
		_pia =  currentMobster FIND objNull;  
		 currentMobster deleteAT _pia;  
	};

	if!(_old isEqualTo  currentMobster) then { publicvariable "currentMobster"; };
	...
	for "_i" from 0 to 20 step 1 do {
		_pia =  currentDMV FIND objNull;  
		 currentDMV deleteAT _pia;  
	};

	if!(_old isEqualTo  currentDMV) then { publicvariable "currentDMV"; };
	...
	for "_i" from 0 to 20 step 1 do {
		_pia =  currentPubDefs FIND objNull;  
		 currentPubDefs deleteAT _pia;  
	};

	if!(_old isEqualTo  currentPubDefs) then { publicvariable "currentPubDefs"; };
	...
	_old =  currentParalegals;

	for "_i" from 0 to 20 step 1 do {
		_pia =  currentParalegals FIND objNull;  
		 currentParalegals deleteAT _pia;  
	};

	if!(_old isEqualTo  currentParalegals) then { publicvariable "currentParalegals"; };
...
if (myJob == "Mafia") then { _allJobs pushBack ["Mafia", currentMafia]; };
if (myJob == "Biker") then { _allJobs pushBack ["Biker", currentBiker]; };
if (myJob == "Mobster") then { _allJobs pushBack ["Mobster", currentMobster]; };
if (myJob == "Cop") then { _allJobs pushBack ["Cop", currentCop]; };
if (myJob == "doc") then { _allJobs pushBack ["DoC", currentdoc]; };
if (myJob == "DA") then { _allJobs pushBack ["DA", currentDA]; };
if (myJob == "Legal") then { _allJobs pushBack ["Legal", currentPubDefs+currentJudges]; };
if (myJob == "EMS") then { _allJobs pushBack ["EMS", currentEMS]; };
if (myJob == "DMV") then { _allJobs pushBack ["DMV", currentDMV]; };
