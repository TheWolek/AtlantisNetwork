...
if(_winnings*((0.75)+((1-casinoRate/100)/5)) > _casinoVault) then {
	...
	[format["Wygrałeś %1 na ostatnim wyścigu koni, ale otrzymałeś tylko %2, ponieważ skończyły się pieniądze w sejfie kasyna.", _winnings call client_fnc_numberText, _winning call client_fnc_numberText],false] spawn domsg;
	...
	[format["Wygrałeś %1 na ostatnim wyścigu koni.", _winning call client_fnc_numberText],false] spawn domsg;
