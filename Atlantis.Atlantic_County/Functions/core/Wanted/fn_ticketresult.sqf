...
if (_status select 7 == 0) exitWith { ["This ticket has already been paid or closed.", true] spawn domsg; };

_cashcheck = [1, _status select 8] call client_fnc_checkmoney;
if!(_cashCheck) exitwith { [format["%1 - You do not have enough money to pay for this!",(_status select 8) call client_fnc_numberText], true] spawn domsg; };
...
[_status select 8] call Client_fnc_removeCash;
["govtBank", _status select 8, "Add"] remoteExec["server_fnc_setValue",2];
