...
if(_arrayCount == 10) then {
	if(_amount > 0) then { if(myJob == "EMS" || myjob == "Fire") then { ["Giving Antibiotics",5,'client_fnc_healTime',_user,"AinvPknlMstpSnonWnonDnon_medic_1",[_user,_myInjuries,_arrayCount,"CG_ATF_Bandage_i"],"cg_mission_files\sounds\patdown1.ogg"] spawn client_fnc_dotask; } else { ["You do not have the required item!", true] spawn domsg; }; };
};	
closedialog 0;
sleep 3;
imhealing = false;
