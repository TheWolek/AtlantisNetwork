...
if(_type == "heroin") then {
	...
	["Możesz bardzo dobrze celować.",false] spawn domsg;
	["Add","unhealthiness",5] call client_fnc_sustain;
	["Remove","Karma",5,"Stress"] call client_fnc_sustain;
};
...
if(_type == "meth") then {
	...
	client_meth = _amount;
	["Czujesz się niezwyciężony.",false] spawn domsg;
	["Add","unhealthiness",5] call client_fnc_sustain;
	["Add","Karma",5,"Stress"] call client_fnc_sustain;
};
...
if(_type == "donuts") then {
	...
	player setUnitRecoilCoefficient 0.5;
	player setCustomAimCoef 0;
	["Czujesz się trwale!",false] spawn domsg;
};
...
[format ["%1 będzie trwać %2m z powodu niezdrowego %3",[_type] call CBA_fnc_capitalize, (_amount * 5), client_unhealthiness],false] spawn domsg;
